...
def get_street(start, neighbourhood, n_filter=lambda x: True):  # for edges
    ...
    def good_neighbour(x):
        return x not in out and n_filter(x)
...
def get_paralell_street(start, adjacent_street, neighbourhood):  # no user filter
    ...
    for other_neighbour in adjacent_street[1:]:
        def good_neighbour(x):
            return x in neighbourhood[other_neighbour] and x not in adjacent_street
...
def quads_to_rows(vertices, indices):
    neighbourhood = {i: set() for i, x in enumerate(vertices)}
    # ^ a map that tells you who your neighbours are
    ...
    def is_edge(point):
        return point in edges

    def end_corner(street):
        return [*filter(lambda neighbour: neighbour in corners, neighbourhood[street[-1]])][0]
...
def generate_solid(rows):
    ...
    dominant_axis = [1 if abs(x) == max(abs(y) for y in N) else 0 for x in N]
...
if __name__ == "__main__":
    ...
    for i, row in enumerate(vector_rows):
        ...
        scope.add("distances")
        scope.add(f"row{i}")
        scope.set_in(base_vmf, " ".join(row_distances))
        scope.retreat()
        scope.retreat()
        scope.add("normals")
        scope.add(f"row{i}")
