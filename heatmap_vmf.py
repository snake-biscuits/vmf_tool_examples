...
# NOTE: heatmaps.tf is dead! use coldmaps instead!
import copy
import json
...
def heatmap_vmf(heatmap, into_vmf="../mapsrc/blank.vmf", victims=True, killers=True, limit=None):
    ...
    for i, tf_class in class_name.items():
        base_vmf.visgroups.visgroups[0].visgroups[2].visgroups.append(vmf_tool.namespace({"name": f"{tf_class}",
                                                                                          "visgroupid": str(80 + i)}))
        base_vmf.visgroups.visgroups[0].visgroups[3].visgroups.append(vmf_tool.namespace({"name": f"{tf_class}",
                                                                                          "visgroupid": str(90 + i)}))
    base_vmf.visgroups.visgroups[0].visgroups[2].visgroups.pop(-1)
    base_vmf.visgroups.visgroups[0].visgroups[3].visgroups.pop(-1)

    # NOTE: there is no downside to adding visgroups and not using them
    base_vmf.visgroups.visgroups[0].visgroups[2].visgroups[5].visgroups = [{"name": "Sentry", "visgroupid": "100"},
                                                                           {"name": "Mini-Sentry", "visgroupid": "101"}]
    ...
    for kill in heatmap["kills"]:
        ...
        try:
            ...
            kw_name = f"UNKNOWN WEAPON #{kw_id}"
        ...
        victim["editor"]["comments"] = f"Killed by {class_name[kill[k_class]]} @ " \
                                       f"{kill[v_x]} {kill[v_y]} {kill[v_z]} with {kw_name}"
        victim["editor"]["visgroupid"] = '"\n\t\t"visgroupid" "'.join([*visgroups, "62"])
        if kill[k_class] == WORLD:
            victim["editor"]["comments"] = "Killed by WORLD"
            victim["angles"] = "180 0 0"
        ...
        if kill[k_class] != WORLD:
            ...
            if kill[k_class] == ENGINEER:
                if kill[k_wep] == SENTRY:
                    ...
                    killer["skin"] = str(kill[k_team] - 2)
                elif kill[k_wep] == MINI_SENTRY:
                    killer["model"] = "models/buildables/sentry1.mdl"
                    killer["skin"] = str(kill[k_team])
            killer["origin"] = f"{kill[k_x]} {kill[k_y]} {kill[k_z]}"
            killer["editor"]["comments"] = f"Killed {class_name[kill[v_class]]} " \
                                           f"@ {kill[k_x]} {kill[k_y]} {kill[k_z]} with {kw_name}"
...
if __name__ == "__main__":
    ...
    # import argparse
    # parser = argparse.ArgumentParser(description="Generate .vmf(s) from heatmaps.tf data",
    #                                  epilog=""@presets.txt" will load args from presets.txt", argument_default=[])
    ...
    import sys
    for filepath in sys.argv[1:]:  # Lazy Drag & Drop
        print("-" * 80)
        print(filepath)
